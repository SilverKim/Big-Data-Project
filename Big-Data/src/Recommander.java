...
public class Recommender
{
	...
	private void updateSupport1(HashSet<Integer> Basket) {
	...
	private void updateSupport2(HashSet<Integer> Basket) {
		if (Basket.size() >= 2) {
	...
	private void updateSupport3(HashSet<Integer> Basket) {
		HashSet<Integer>
	...
	private int predictPair(HashSet<Integer> profile, Integer q) {
	...
	private int predictTriple(HashSet<Integer> profile, Integer q) {
		if (profile.size() < 2)
			return 0 ;
...
class IntTriple implements Comparable
{
	int [] elem ;

	IntTriple(Set<Integer> s) {
		/* TODO: implement this method */
		Integer [] i = s.toArray(new Integer[3]);
		Arrays.sort(i);
		elem = new int[3];
		for(int index = 0; index < 3; index++){
			elem[index] = i[index].intValue();
		}
	...
	public int compareTo(Object obj) {
		...
		for(int index = 0; index < 3; index++) {
			if(this.elem[index] < t.elem[index])
				return -1;
			if(this.elem[index] > t.elem[index])
				return 1;
		}
